...
- ✅ Транскрибация голосовых сообщений через Whisper (на удаленном сервере)
...
2. Отредактируйте `.env` файл:
   ...
   **Настройка Whisper (для голосовых сообщений):**
   ```bash
   WHISPER_MODEL=base
   SSH_HOST=195.209.210.129
   SSH_USER=ubuntu
   SSH_PORT=22
   SSH_KEY_FILE=.ssh/immers-vm.pem
   ```
...
- `WHISPER_MODEL` - модель Whisper для транскрибации (tiny, base, small, medium, large). По умолчанию: base
- `SSH_HOST` - IP адрес или хост сервера с Whisper (обязательно для голосовых сообщений)
- `SSH_USER` - пользователь для SSH подключения (по умолчанию: ubuntu)
- `SSH_PORT` - порт SSH (по умолчанию: 22)
- `SSH_KEY_FILE` - путь к SSH ключу для подключения к серверу (по умолчанию: .ssh/immers-vm.pem)
...
Бот поддерживает транскрибацию голосовых сообщений через Whisper на удаленном сервере:
- Отправьте голосовое сообщение в бот
- Бот автоматически транскрибирует его через Whisper на удаленном сервере и обработает как текстовое сообщение
- Для работы требуется настроить SSH подключение к серверу с установленным Whisper
- Рекомендуется модель `base` для баланса скорости и точности
- Whisper должен быть установлен на удаленном сервере в виртуальном окружении `~/whisper-env`
...
- Транскрибация голосовых сообщений выполняется на удаленном сервере через SSH
- Для работы требуется настроить SSH подключение к серверу с установленным Whisper

### Настройка Whisper на сервере

Whisper должен быть установлен на удаленном сервере в виртуальном окружении `~/whisper-env`.

**Требования на сервере:**
- Python 3.11+
- ffmpeg (установлен через `apt-get install ffmpeg` или аналогично)
- Виртуальное окружение `~/whisper-env` с установленным `openai-whisper`
- Предзагруженная модель `base` (автоматически загружается при первом использовании)
...
│   └── transcription.py # Транскрибация через Whisper на удаленном сервере
├── prompts/

...
async def handle_voice(message: Message):
    ...
    try:
        ...
        logger.info(f"Downloading voice file: {file_info.file_path}")
        file_buffer = await message.bot.download_file(file_info.file_path)
        ...
        try:
            ...
            logger.info(f"Transcribing voice message from {chat_id}...")
        ...
        logger.error(f"File not found error for voice from {chat_id}: {e}", exc_info=True)
        await message.answer(
            "Не удалось скачать голосовое сообщение. "
            "Попробуйте отправить его еще раз."
        )
    except RuntimeError as e:
        logger.error(f"Transcription error for voice from {chat_id}: {e}", exc_info=True)
        error_msg = str(e)
        if "SSH" in error_msg or "connection" in error_msg.lower():
            await message.answer(
                "Не удалось подключиться к серверу транскрипции. "
                "Пожалуйста, попробуйте позже или используйте текстовое сообщение."
            )
        else:
            await message.answer(
                "Произошла ошибка при транскрипции голосового сообщения. "
        ...
        await message.answer(
            "Транскрипция прошла успешно, но произошла ошибка при обработке текста. "
            "Пожалуйста, попробуйте еще раз через несколько секунд."
        )
    except Exception as e:

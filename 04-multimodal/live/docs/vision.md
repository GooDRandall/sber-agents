...
- **openai-whisper** - локальная транскрибация голосовых сообщений (хостится на том же сервере, где Ollama)
...
│   └── transcription.py # Транскрибация голосовых сообщений через Whisper
├── prompts/
...
2. **handlers.py** - обработка событий
   ...
   - Обработчик голосовых сообщений → транскрибация через Whisper → извлечение транзакций через LLM → сохранение транзакций → показ ответа + статус + баланс
...
5. **config.py** - конфигурация
   - Класс Config с полями: `TELEGRAM_TOKEN`, `OPENAI_API_KEY`, `OPENAI_BASE_URL`, `MODEL_TEXT`, `MODEL_IMAGE`, `SYSTEM_PROMPT_TEXT`, `SYSTEM_PROMPT_IMAGE`, `WHISPER_MODEL`
   ...
   - `WHISPER_MODEL` - размер модели Whisper для транскрибации (tiny, base, small, medium, large)

6. **transcription.py** - транскрибация голосовых сообщений
   - Метод `transcribe_audio()` - транскрибация аудиофайла через локальный Whisper
   - Whisper хостится локально на том же сервере, где работает Ollama (модель для изображений)
   - Поддержка различных аудиоформатов (конвертация в нужный формат при необходимости)
   - Кэширование модели Whisper для повышения производительности
   - Локальная установка и работа, аналогично Ollama
...
```
Telegram → handlers.py (голосовое сообщение → скачать файл) → transcription.py (Whisper локально на сервере) → 
transcription.py (текст транскрипции) → handlers.py → llm.py (structured output) → OpenRouter/Ollama (локально) → 
llm.py → handlers.py (извлечь транзакции, сохранить в transactions, показать ответ + статус + баланс) → Telegram
```
...
1. Пользователь отправляет голосовое сообщение (например: "Сегодня купил продукты на 1500 рублей")
2. Бот скачивает аудиофайл из Telegram
3. Бот транскрибирует аудио через локальный Whisper → получает текст
4. Бот отправляет транскрибированный текст в LLM со structured output
5. LLM извлекает транзакции из текста
6. Бот сохраняет транзакции в `transactions[chat_id]`
7. Бот рассчитывает баланс
8. Бот отправляет пользователю:
   - Транскрибированный текст (опционально)
   - Ответ LLM из поля `answer`
   - Статус: "Найдено и сохранено N транзакций"
   - Текущий баланс
...
- Транскрибация голосовых сообщений требует локальных вычислительных ресурсов (CPU/GPU) на том же сервере, где хостится Ollama
...
# Whisper хостится локально на том же сервере, где Ollama (модель для изображений)
```

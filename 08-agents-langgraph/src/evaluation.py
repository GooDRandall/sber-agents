...
# Импорт Ollama embeddings (опционально, если используется)
try:
...
def create_ragas_embeddings():
    ...
    Поддерживает: openai, huggingface, ollama
    """
    ...
    if provider == "openai":
        ...
        logger.info(f"Creating RAGAS Ollama embeddings: {config.RAGAS_EMBEDDING_MODEL} at {config.OLLAMA_BASE_URL}")
        ...
        raise ValueError(f"Unknown RAGAS embedding provider: {provider}. Use 'openai', 'huggingface', or 'ollama'")
...
def init_ragas_metrics():
    ...
    if config.RAGAS_LLM_PROVIDER == "ollama":
        ollama_base_url = config.OLLAMA_BASE_URL.rstrip('/')
        if not ollama_base_url.endswith('/v1'):
            ollama_base_url = f"{ollama_base_url}/v1"
        llm_kwargs["base_url"] = ollama_base_url
        llm_kwargs["api_key"] = "ollama"  # Заглушка, Ollama не требует ключ
    ...
    elif config.RAGAS_EMBEDDING_PROVIDER == "ollama":
        logger.info(f"✓ RAGAS Embedding Model: {config.RAGAS_EMBEDDING_MODEL} at {config.OLLAMA_BASE_URL}")

...
def get_rag_chain():
    ...
    async def transform_and_retrieve(inputs):
        """Трансформация запроса с логированием и поиск"""
        ...
        # Логируем преобразованный запрос
        original_query = messages[-1].content if messages else "N/A"
        logger.info(f"Query transformation: '{original_query}' -> '{transformed_query}'")
        
        # Выполняем поиск
        chunks = await retriever.ainvoke(transformed_query)
    ...
    return (
        RunnablePassthrough.assign(
            context=transform_and_retrieve
        )

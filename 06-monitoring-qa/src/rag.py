...
def reset_llm_cache():
    """Сброс кеша LLM клиентов (полезно при изменении конфигурации)"""
    global _llm_query_transform, _llm
    _llm_query_transform = None
    _llm = None
...
def _get_llm_query_transform():
    ...
    if _llm_query_transform is None:
        if config.LLM_PROVIDER == "ollama":
            ...
            _llm_query_transform = ChatOpenAI(**kwargs)
        logger.info(f"Query transform LLM initialized: {config.MODEL_QUERY_TRANSFORM} (provider: {config.LLM_PROVIDER})")
...
def _get_llm():
    ...
    if _llm is None:
        ...
        logger.info(f"Main LLM initialized: {config.MODEL} (provider: {config.LLM_PROVIDER})")
    return _llm

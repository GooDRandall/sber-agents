...
def load_and_sample_pdf_chunks(data_dir: str, samples_per_file: int = 10) -> List:
    """
...
def synthesize_qa_pairs_from_pdf(chunks: List, llm_model: str = None) -> List[Dict[str, Any]]:
    """
    Синтез вопросов и ответов из PDF чанков через LLM
    
    Args:
        chunks: список чанков документов
        llm_model: модель для синтеза (если None, используется из config)
    ...
    if llm_model is None:
        llm_model = config.MODEL or config.RAGAS_LLM_MODEL or "gpt-4o"
    
    # Определяем провайдер на основе используемой модели
    # Если модель из MODEL, используем LLM_PROVIDER, иначе RAGAS_LLM_PROVIDER
    if llm_model == config.MODEL:
        llm_provider = config.LLM_PROVIDER
    else:
        llm_provider = config.RAGAS_LLM_PROVIDER
